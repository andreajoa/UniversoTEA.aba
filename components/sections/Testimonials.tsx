    ...
    <section ref={ref} className="bg-white py-16 px-4">
      <div className="max-w-5xl mx-auto flex flex-col items-center gap-10">
        ...
        <motion.div initial={{ opacity: 0 }} animate={inView ? { opacity: 1 } : {}} transition={{ duration: 0.6, delay: 0.5 }}
          className="grid grid-cols-2 sm:grid-cols-4 gap-3 md:gap-4 w-full px-4">
          {["🧪 Baseado em ABA", "🖨️ Pronto para imprimir", "📲 Acesso imediato", "🎓 Aprovado por terapeutas"].map((badge, i) => (
            <div key={i} className="bg-blue-50 text-primary font-display font-bold text-xs sm:text-sm md:text-sm text-center px-2 sm:px-3 md:px-4 py-2 sm:py-3 rounded-xl md:rounded-2xl">
              {badge}
            </div>
          ))}
        </motion.div>
